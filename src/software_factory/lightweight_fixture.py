...
class FixtureModelGateway:
    """Deterministic compatibility fixture for the lightweight Python profile only."""

    async def complete(self, schema: type[T], *, system: str, user: str) -> T:
        if TargetProfile.ENTERPRISE_DOTNET_REACT.value in user:
            raise ValueError(
                "Enterprise fixture requests must use SpecDrivenFixtureModelGateway; "
                "legacy lightweight fixtures are not an enterprise generation path."
            )
        if schema is RequirementSpec:
            value: BaseModel = _lightweight_requirements()
        elif schema is ArchitectureSpec:
            value = _lightweight_architecture()
        elif schema is ApplicationSpec:
            value = _lightweight_application_spec()
        elif schema is TaskPlan:
            value = _lightweight_task_plan()
        elif schema is ArtifactSet:
            value = _fixture_artifacts(system)
        elif schema is CodeBundle:
            value = _lightweight_bundle()
        else:
...
def _lightweight_bundle() -> CodeBundle:
    artifacts = [
